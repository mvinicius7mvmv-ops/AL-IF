...
import { normalizePhone, POSITIONS, cn } from '@/lib/utils';
import {
  Plus,
  Users,
  Search,
...
export function AdminPlayers() {
  ...
  // Abre "Novo Jogador" caso a URL tenha ?novo=1
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    const normalized = form.telefone
      ? normalizePhone(form.telefone)
      : '';

    if (form.telefone && normalized.length < 10) {
    ...
    try {
      ...
      // CRIAR JOGADOR
        ...
        if (!resp.ok) {
          throw new Error(
            result.error || 'Erro ao criar jogador'
          );
        }

        // Upload da foto após criar o jogador
        ...
        setGeneratedCreds({
          telefone: result.credentials.telefone,
      ...
      showToast(
        e.message || 'Erro ao salvar jogador',
        'error'
      );
    } finally {
      setSaving(false);
  ...
  async function handleToggleStatus(p: Profile) {
    try {
      ...
      showToast(
        e.message || 'Erro ao alterar status',
        'error'
      );
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      const apiUrl =
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/delete-player`;
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Clique em 'Novo Jogador' para começar."
        />
      ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-lg bg-green-900/20 border border-green-800/50">
              ...
              <p className="text-neutral-300 text-sm">
                Envie as credenciais abaixo ao jogador via WhatsApp para o primeiro acesso.
              </p>
            ...
            <p className="text-neutral-500 text-xs">
              A senha inicial são os últimos 4 dígitos do telefone. O jogador será obrigado a trocar a senha no primeiro acesso.
            </p>
      ...
      <ConfirmModal
        ...
        title="Redefinir senha"
        message={
          `Tem certeza que deseja redefinir a senha de ${resetTarget?.nome}? ` +
          `Uma nova senha temporária será criada e o jogador deverá definir ` +
          `uma nova senha no próximo acesso.`
        }
        confirmLabel={
          resettingPassword
            ? 'Redefinindo...'
      ...
      <ConfirmModal
        ...
        message={
          ...
          `Esta ação removerá o jogador e sua conta de acesso. ` +
      ...
      >
        ...
        {resetCreds && (
          ...
          <div className="space-y-4">
            ...
            <p className="text-neutral-500 text-xs">
              Esta é uma senha temporária. No próximo acesso, o jogador será obrigado a cadastrar uma nova senha.
            </p>
