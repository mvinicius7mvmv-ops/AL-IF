...
export function AdminCompetitions() {
  ...
  async function handleLogo(file: File) {
    console.log('[Competitions] handleLogo iniciado:', { name: file.name, type: file.type, size: file.size });
    setUploading(true);
    try {
      ...
      if (!allowedExt.includes(ext || '') && !allowedMime.includes(mime)) {
        console.warn('[Competitions] Formato não suportado:', { name: file.name, type: file.type, ext });
      ...
      const path = `competitions/${Date.now()}.${safeExt}`;
      ...
      console.error('[Competitions] Erro no upload:', e);
  ...
  return (
    <div className="space-y-5">
      ...
      <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/jpg,image/webp,image/svg+xml,image/*" className="sr-only" onChange={e => { const f = e.target.files?.[0]; if (f) handleLogo(f); e.target.value = ''; }} />
