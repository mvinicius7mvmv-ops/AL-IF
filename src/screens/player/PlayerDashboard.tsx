...
import { Calendar, Wallet, TrendingUp, Goal, Shirt, ArrowRight } from 'lucide-react';
...
export function PlayerDashboard() {
  ...
  const [myStats, setMyStats] = useState({ gols: 0, assistencias: 0, jogos: 0, presenca: 0 });
  ...
  async function load() {
    ...
    try {
      ...
      const [matchesRes, attRes, eventsRes, feesRes] = await Promise.all([
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-sm font-semibold text-neutral-400 uppercase tracking-wide mb-3">Minhas Estatísticas</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatBox icon={<Goal size={16} />} label="Gols" value={myStats.gols} />
          <StatBox icon={<TrendingUp size={16} />} label="Assistências" value={myStats.assistencias} />
          <StatBox icon={<Shirt size={16} />} label="Jogos" value={myStats.jogos} />
          <StatBox icon={<Calendar size={16} />} label="Presenças" value={myStats.presenca} />
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-neutral-400 uppercase tracking-wide">Próximos Jogos</h2>
